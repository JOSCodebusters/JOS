...
mem_init_mp(void)
{
	...
	for (i = 0; i < NCPU; i++) 
	{
		boot_map_region(kern_pgdir, temp_top - KSTKSIZE, KSTKSIZE, PADDR(percpu_kstacks[i]), PTE_W | PTE_P);
		temp_top -= (KSTKSIZE + KSTKGAP);
	}
...
//
// Reserve size bytes in the 0MMIO region and map [pa,pa+size) at this
...
mmio_map_region(physaddr_t pa, size_t size)
{
	...
	// Your code here:
	uintptr_t temp=base;
	size_t temp_size=ROUNDUP(size,PGSIZE);
	if(base+temp_size>MMIOLIM)
		panic("Out of Range!");
	boot_map_region(kern_pgdir, base, temp_size, pa, PTE_PCD|PTE_PWT|PTE_W);
	base+=temp_size;
	return (void *)temp;
