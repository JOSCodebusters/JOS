...
trap_init(void)
{
	...
	SETGATE(idt[0], 0, GD_KT, d, 0);
	SETGATE(idt[1], 0, GD_KT, debug, 0);
	SETGATE(idt[2], 0, GD_KT, nonmsakable, 0);
	SETGATE(idt[3], 0, GD_KT, dreakpoint, 3);
	SETGATE(idt[4], 0, GD_KT, overflow, 0);
	SETGATE(idt[5], 0, GD_KT, bounds, 0);
	SETGATE(idt[6], 0, GD_KT, illegal, 0);
	SETGATE(idt[7], 0, GD_KT, device, 0);
	SETGATE(idt[8], 0, GD_KT, doublef, 0);
	SETGATE(idt[10], 0,GD_KT, itss, 0);
	SETGATE(idt[11], 0,GD_KT, snp, 0);
	SETGATE(idt[12], 0,GD_KT, se, 0);
	SETGATE(idt[13], 0,GD_KT, gpf, 0);
	SETGATE(idt[14], 0,GD_KT, pf, 0);
	SETGATE(idt[16], 0,GD_KT, fpe, 0);
	SETGATE(idt[17], 0,GD_KT, ac, 0);
	SETGATE(idt[18], 0,GD_KT, mc, 0);
	SETGATE(idt[19], 0,GD_KT, simd, 0);
...
page_fault_handler(struct Trapframe *tf)
{
	...
	if (UXSTACKTOP-PGSIZE<=tf->tf_esp && tf->tf_esp<=UXSTACKTOP-1)
            temp_ptr = tf->tf_esp - sizeof(struct UTrapframe) -sizeof(uintptr_t);
        else 
            temp_ptr = UXSTACKTOP - sizeof(struct UTrapframe);
            
        struct UTrapframe* temp_frame = (struct UTrapframe*) temp_ptr;
        user_mem_assert (curenv, (void *)temp_frame, sizeof(struct UTrapframe), PTE_W);
        //cprintf("size3=%d",sizeof(temp_frame->utf_regs));
        temp_frame->utf_fault_va = fault_va;
	temp_frame->utf_err  = tf->tf_err;
	//cprintf("%d\n",tf->tf_err);
	temp_frame->utf_regs = tf->tf_regs;
	temp_frame->utf_eip  = tf->tf_eip;
	temp_frame->utf_eflags = tf->tf_eflags;
	temp_frame->utf_esp = tf->tf_esp;
	curenv->env_tf.tf_eip = (uintptr_t)curenv->env_pgfault_upcall;
	curenv->env_tf.tf_esp =  temp_ptr; 
	env_run(curenv);
