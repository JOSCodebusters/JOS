...
//	if(req->req_n > PGSIZE)
	...
	ssize_t count = file_read(f, ret->ret_buf, req->req_n, fd->fd_offset);
	if(count < 0)
		return count;
	
	fd->fd_offset = fd->fd_offset + count;
...
serve_write(envid_t envid, struct Fsreq_write *req)
{
	...
	if (fd->fd_offset + req->req_n > f->f_size)
        f->f_size = fd->fd_offset + req->req_n;
		
	ssize_t count = file_write(f, req->req_buf, req->req_n, fd->fd_offset);
	if(count < 0)
		return count;
