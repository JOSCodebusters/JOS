...
trap_init(void)
{
 ...
 /*   void (*funs[256])();
	...
	SETGATE(idt[48], 0, GD_KT, funs[48], 3);*/			//was trying the challenge in lab 3 :-(
...
page_fault_handler(struct Trapframe *tf)
{
	...
	if(curenv->env_pgfault_upcall){
		...
		if((tf->tf_esp >= UXSTACKTOP - PGSIZE) && (tf->tf_esp <= UXSTACKTOP-1))
			utf_address = tf->tf_esp - sizeof(struct UTrapframe) - 4;
		else
			utf_address = UXSTACKTOP - sizeof(struct UTrapframe);
		
		user_mem_assert(curenv, (void *)utf_address, 1, PTE_W);
		
		utf = (struct UTrapframe *)utf_address;
		
		/* information about the fault */
		utf->utf_fault_va = fault_va;	/* va for T_PGFLT, 0 otherwise */
